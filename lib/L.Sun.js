// the original code is https://github.com/dj0001/Leaflet.Sun/blob/master/L.Sun.js is modified by i-namekawa

function sun(Breite,Laenge,T,s, td)   //s=1=sunset; s=-1=sunrise; 
// i-namekawa: td as arg added.
{
//   var Breite = latlon.lat, Laenge = latlon.lng, T = new Date(), s = s||1
  ...
  Untergang = Untergang - Laenge / 15.0 + td;  // i-namekawa: used ts here instead of 'new Date().getTimezoneOffset() / 60'
  Untergang = Math.floor(Untergang) % 24 + ":" + ("0" + Math.round(Untergang % 1 * 60)).slice(-2) // i-namekawa: adding td can go beyond 24 hour
  return Untergang
}  
